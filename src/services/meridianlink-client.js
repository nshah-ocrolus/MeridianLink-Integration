 ...
 *
 * Wraps MeridianLink Mortgage SOAP web services with OAuth 2.0 authentication.
 *
 * Authentication flow (from official docs):
 *   POST https://secure.mortgage.meridianlink.com/oauth/token
 *   Body: { client_id, client_secret }
 *   Returns: { access_token, token_type: "Bearer", expires_in: 14400 }
 *
 * The access_token is passed as sTicket in SOAP calls: "Bearer {token}"
 *
 * Service domains (from Getting Started docs):
 *   Auth/Loan: webservices.mortgage.meridianlink.com/los/webservice/
 *   EDocs:     edocs.mortgage.meridianlink.com/los/webservice/
 *
...
class MeridianLinkClient {
  constructor() {
    ...
    this.authDomain = config.meridianlink.authDomain;
    this.edocsDomain = config.meridianlink.edocsDomain;
  ...
  /**
   * Get a valid sTicket for SOAP calls.
   ...
   * Domain: edocs.mortgage.meridianlink.com
   */
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  <soap:Body>
    ...
    const response = await axios.post(
      `${this.edocsDomain}${WS_PATH}/EDocsService.asmx`,
      soapEnvelope,
      {
        headers: {
